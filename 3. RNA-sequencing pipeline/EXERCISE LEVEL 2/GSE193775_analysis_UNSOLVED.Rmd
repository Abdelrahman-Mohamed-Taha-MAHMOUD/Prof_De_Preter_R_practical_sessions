---
title: "Differential Expression - exercise LEVEL 2"
...
## Differential Expression between YAP1-mutant versus control transduced cell lines

The BRAFV600E mutant melanoma cell lines A375 and SKMEL28 were transduced with a lentiviral vector expressing a constitutively active mutant of YAP1 (YAP5SA) to induce YAP1 activity.
In this notebook, we will perform a differential expression (DE) analysis between transduced cell lines versus non-transduced cell lines. So, we will identify the genes that have a higher or lower expression upon YAP1 lentiviral tranduction. This analysis will be performed with DESeq2. Limma Voom is an alternative method, amongst others. After DE analysis, we will see which genesets are up- or downregulated.
You can look up the study from which we are using the data by searching for the GSE193775 dataset in Gene Expression Omnibus and the paper via PubMed (PMID: 35798875).
...
Read in the sample information file as tibble and then transform to a dataframe with the "treatment" (Control vs YAP1mut) information in the first column.
...
This design tells to DESeq what sample groups you want to compare (in our case we compare Control versus YAP1mut). This command give a warning message telling you that characters are converted to factors, you can ignore this message.
...
Filter data to give your test more power. In this case, we will only keep the genes with more than 20 counts when we sum the counts over all samples. Do the filtering and check how many genes you keep.
...
Using the function estimateSizeFactors you can normalize the raw RNA-seq counts using the median ratio method. After that, we log transform the normalized count data. 
...
Similar as for PCA we perform hierarchical clustering and heatmap visualization to explore the similarities between the samples. Write the script for the second option using "euclidean" distance and "ward.D2" clustering.
...
## Second option: a heatmap including hierarchical clustering visualised with a dendrogram (distance = euclidean and clustering method = ward.D2)
...
If you use the DESeq comment on the counts you will perform normalization and differential expression analysis in one go. The different steps done by DESeq will pop up in your console. Size factors and dispersion are estimated for normalization purposes. Afterwards the model/design is fit and all genes are tested for differential expression. In this case, the design is very simple (comparing Control vs YAP1mut), but you can make much more complex designs (not part of this course), e.g. in this example we could have taken into account the cell_line info (however this requires a bit more complex design which we will therefore not show in this practical session).
...
With the results function we can extract a table with Fold changes, P-values, ... from the dds object. Standard, a cut-off of 0.1 is used, but we can be stricter if we want by changing some of the parameters. Use padj < 0.01 and log2(fold-change) of more than 1 to select differentially expressed genes.
...
## Plot the expression of the most differential gene

Plot the expression of the 3 genes with the lowest p-adjusted value. What is the gene name (gene symbol) of the most differentially expressed gene?
...
# Gene set enrichment analysis (GSEA) (OPTIONAL PART, NO EXERCISES IN THIS SECTION)
...
We could also use visualisations implemented in the fgsea package itself. Here we select the top and bottom genesets based on NES value (ignoring padj values).
