---
title: "Differential Expression - EXERCISE_DIFFICULTY LEVEL 3"
...
## Differential Expression between DMSO vs Toyocamycin treated cells

You can look up the study from which we are using the data by searching for the GSE202744 dataset in Gene Expression Omnibus and the paper via PubMed (PMID: 35884401).
...
We read in the sample information file as tibble and then transform to a dataframe with the "treatment" (Control vs YAP1mut) information in the first column.
...
With the results function we can extract a table with Fold changes, P-values, ... from the dds object. Default, a cut-off of 0.1 is used, but we can be stricter if we want by changing some of the parameters (e.g., padj < 0.05 and log2(fold-change) of more than 1).
...
REMARK: We observe that the samples after only 2 hours of treatment do not differ a lot in transcriptional profile compared to untreated cells (similar as in the publication). Based on this you might decide to exclude these samples in a final differential expression analysis (optional, not part of this Rmarkdown exercise).
...
With this code, we plot the expression of the gene with the lowest p-adjusted value. What is the gene name (gene symbol) of the most differentially expressed gene?
...
In this part we will perform GSEA on both H (hallmark genesets) as on C6 (oncogenic signature genesets).
...
## Get gene set data OPTIONAL (ONCO C6)
...
## Visualize significant GSEA OPTIONAL (ONCO C6)
...
REMARK: Here you can see that only significant upregulated genesets are identified using GSEA (up in treated cells versus untreated). KRAS signaling seems to be an important pathway affected, however this is not mentioned in the publication. 
