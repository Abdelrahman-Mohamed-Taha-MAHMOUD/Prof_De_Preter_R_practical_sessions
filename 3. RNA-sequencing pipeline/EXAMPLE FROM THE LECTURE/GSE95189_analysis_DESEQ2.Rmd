...
However, `fgsea` (fgsea Fast Gene Set Enrichment Analysis) needs the database as a list, not as a data frame. There are many ways we can accomplish this. Here is one using the `tidyverse`. Remember to select the gene ID column that matches your fold change data!
...
GSEA Gene set enrichment analysis (GSEA) orders the genes based on the mean gene expression fold change between the two groups that are compared. Thus, we retrieve it from the DE results in 'res'. 
...
Using our formatted data, we run GSEA. We also set the number of permutations to run with `nperm`, (nperm number of permutations used for running the gene set enrichment analysis) which determines how many times the algorithm is run to estimate p-values.
