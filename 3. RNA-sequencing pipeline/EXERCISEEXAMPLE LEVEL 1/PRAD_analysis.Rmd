---
title: "Differential Expression - Example/Exercise LEVEL 1"
...
## Differential Expression between tumor and normal samples

In this notebook, we will perform a differential expression (DE) analysis between tumor samples of prostate cancer patients and normal adjacent tissue. So, we will identify the genes that have a higher or lower expression in prostate cancer tissue versus normal prostate tissue. This analysis will be performed with DESeq2. Limma Voom is an alternative method, amongst others. After DE analysis, we will see which genesets are up- or downregulated.
...
Our count data is saved as a tab separated file. We need to use a read function that uses tabs as a seperator.
We need to combine to files and end with a data frame that has the samples as column names and genes as rownames.
Inspect the CountTable with view.
...
RNAseq_data <- read_tsv("PRAD_data.txt") %>% column_to_rownames("GeneID")
```

## Reading in the sample information

We read in the sample information file as tibble and then transform to a dataframe with the "Conditions" (normal or tumor) information in the first column.

```{r}
AnnotationTable <- read_tsv("AnnotationTable_PRAD.txt") %>% column_to_rownames("SampleID")
```
...
This design tells to DESeq what sample groups you want to compare (in our case we compare tumor vs normal). This command give a warning message telling you that characters are converted to factors, you can ignore this message.
...
```{r}
### We first select only the data from the top 1000 genes (heatmap visualization on all genes with be too computer intensive)
...
## First option: just a heatmap without hierarchical clustering (check all arguments in this function). Data are standardized using the function scale.
...
If you use the DESeq comment on the counts you will perform normalization and differential expression analysis in one go. The different steps done by DESeq will pop up in your console. Size factors and dispersion are estimated for normalization purposes. Afterwards the model/design is fit and all genes are tested for differential expression. In this case, the design is very simple (comparing tumor vs normal), but you can make much more complex designs (not part of this course).
...
GSEA orders the genes based on the mean gene expression fold change between the two groups that are compared. Thus, we retrieve it from the DE analysis in `res``. 
