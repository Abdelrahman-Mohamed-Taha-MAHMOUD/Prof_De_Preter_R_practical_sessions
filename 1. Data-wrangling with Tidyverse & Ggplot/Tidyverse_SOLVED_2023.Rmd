...
Welcome in the second practical session. To help you out, you can find a cheatsheet [here](https://images.datacamp.com/image/upload/v1676302697/Marketing/Blog/Tidyverse_Cheat_Sheet.pdf), these are summaries of the most useful functions for tidyverse and ggplot. It helps to print them out, and use them when making the exercises.

# About R, Rstudio, RMarkdown and jupyter notebooks

For these practical sessions you will use RMarkdown in Rstudio. You can set up by first installing [R](https://cran.r-project.org) and afterwards [Rstudio](https://rstudio.com/products/rstudio/download/). If you're using this software later for other projects, don't forget to update this on a regular basis. Another option is to go to Athena and run Rstudio from there, than you don't need to install anything. We posted a short video on UFora that demonstrates you how to work with RStudio.

# Data exploration

Data exploration and visualization of your data is an important first step and can lead to new hypotheses that are worthwhile testing or validating (in the lab or on your computer). The goal of data exploration is to generate interesting new leads that you can later explore in more depth. In this practical session you will learn how to import, tidy and transform your data into the desired format. In the second part of this practical session, you will go deeper into data visualization. At the end of this session, you should be able to make publication ready graphs. In later session, you will learn how to perform statistical testing and learn some new, more advanced, approaches such as survival analysis. Finally, you will learn how a RNA-sequencing data-mining pipeline can be performed using R.
...
* dplyr: helps you out with data manipulation challenges
...
If you use Rstudio from Athena, Tidyverse is already pre-installed. If you use Rstudio on your local computer, you might need to install tidyverse first by using the `install.packages()` function (in that case remove the # in the script before before running). Each time you start a new R session, it's needed to load the tidyverse library (by running `library(tidyverse)`) if you want to work with it. The installation is only needed to perform once.
...
There are a couple of main differences between a tibble and a classic data.frame (as used in base R). When you print a tibble, you will only see the first few lines, while for a data.frame you see the whole structure. A second big difference is when extracting subsets from your data. Tibbles are more strict and will give you error messages when the column is not existing. Tibbles usually don't assign rownames, if you do so (this can be forced), than two rows cannot be assigned the same name (which is similar as for dataframes). Tibbles can have column names that contain spaces or other characters that are not always allowed as variables in dataframes baseR. In the next block of code, you discover how to create tibbles.
...
If you want to perform some data analysis, the first thing you need to do is reading in the data in R. With readr, a package that is contained in tidyverse (it means that you don't have to load readr if you have already loaded tidyverse), you can read comma separated files (.csv, by: `read_csv()`), tab separated files (.tsv, by: `read_tsv()`), tabular files where columns are separated by a white space (by: `read_table()`) and others. You will notice that these functions are similar to their base R analogs (eg. `read.table()`, `read.tsv()`, ...) but when using the "_"/tidyverse functions, the data is directly converted to a tibble (= the tidyverse compatible data structure).

## The Neuroblastoma Dataset

For this practical session, we will work with two data sets. First, we will use a neuroblastoma patient dataset (neuroblastoma is a rare childhood cancer), containing information about the clinical stage of the patient (1 is a local tumor, 4 is a metastatic tumor, 4S is a special stage specific to neuroblastoma), the age of the patient, the overall survival (OS) and event free survival (EFS) information, MYCN amplification status and expression of two genes (MYC and MYCN).
...
```{r}
NB_data <- read_csv("Data/NB_data.csv")
```

There are several steps we could take to explore our data. We can just look to the first few lines. You can change this to see more lines (eg. 20). 
...
Secondly, we will control if the data types of each column are correctly assigned. You can use this by the `str()` function. We see that some columns have the data type 'col_double', this stands for double precision numeric values which is similar to the floating numbers in Python (numeric data including those with decimal points).
What is the data type of stage, MYCN_status and MYC_expr in the tibble? Is this what you expect? 
We note that MYCN_status, OS_bin and EFS_bin are assigned numeric (col_double()), instead of factor type. These variables have only two values, namely 0 and 1. Since these values stand for a particular status, eg. MYCN amplifications or no MYCN amplification, and not a numerical/integer value, we want them to be treated as a status and thus we need to convert these data to a factor. Factors represent categorical data that are either nominal or ordinal. You will see that the data haven’t changed in the tibble (still 0 and 1 values), however these data are now treated as categorical data or factor. The advantage of storing data as factors over characters, … is that it's more memory friendly, so your computer/calculations will work faster. 

[Here](https://biocorecrg.github.io/CRG_R_tidyverse_2021/data-types-in-tibbles.html) you can find some further information on the different data-types that exist.
...
NB_data <- read_csv("Data/NB_data.csv", col_types = list(col_character(), col_factor(), col_character(), col_double(), col_double(), col_factor(), col_double(), col_factor(), col_double(), col_double()))
```
...
Instead of downloading the data, we can directly download and read the data from the web by using the curl library with the `curl()` function.
...
#covid_hospitalisations_data <- read_csv(curl("https://epistat.sciensano.be/Data/COVID19BE_HOSP.csv")) (we are not running this line anymore as apparantly the new data available via the website have been updated and it now results in a strange result in one of the exercises. Therefore, we use the covid-hospitalisation data that you have uploaded from the local file.)
...
```{r}
write_tsv(covid_mortality_data, "Data/covid_mortality.tsv")

write_csv(covid_mortality_data, "Data/covid_mortality.csv")
```
...
covid_mortality_data %>%
  filter(AGEGROUP == "85+" & SEX == "F")

# filter the data with deaths in the 0-24 age group
covid_mortality_data %>%
  filter(AGEGROUP == "0-24")

# filter the data in the 0-24 and 85+ age group
covid_mortality_data %>%
  filter(AGEGROUP %in% c("0-24","85+"))

# filter the data with more than 20 deaths in Wallonia
covid_mortality_data %>%
  filter(DEATHS > 20 & REGION == "Wallonia")

# filter the data with more than 20 deaths in Flanders or more than 10 deaths in Brussels
covid_mortality_data %>%
  filter((DEATHS > 20 & REGION == "Flanders") | (DEATHS > 10 & REGION == "Brussels"))
```
...
covid_tests_data %>%
  mutate(ratio_ptests = TESTS_ALL_POS / TESTS_ALL)

# mark all entry points/rows with more than 200 positive tests as "high risk days"
covid_tests_data %>%
  mutate(risk = case_when( TESTS_ALL_POS >= 200 ~ "high",
                           TRUE~"low"))
```
...
```{r}
covid_tests_data %>%
  filter(DATE >= "2020-09-01" & REGION == "Flanders") %>%
  group_by(PROVINCE) %>%
  summarise(sumPOSTESTS = sum(TESTS_ALL_POS))
```
...
covid_tests_data %>%
  group_by(DATE) %>%
  summarise(sumTESTS_ALL= sum(TESTS_ALL), sumTESTS_ALL_POS = sum(TESTS_ALL_POS)) %>%
  mutate(POSRATIO = sumTESTS_ALL_POS/sumTESTS_ALL) %>%
  filter(POSRATIO > 0.2)
```
...
For our COVID-data we could merge the cases with the tests based on the date. The problem is that the covid cases don't distingish between the sexes and age groups. Therefore, we will first convert the COVID cases table to by using group_by and summarise as we have seen before.
...
```{r}
left_join(covid_mortality_data %>%
            group_by(DATE, REGION) %>%
