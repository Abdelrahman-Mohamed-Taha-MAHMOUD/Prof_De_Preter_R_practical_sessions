...
```{r}
M <- matrix(c(4, 8, 9, 4, 10, 5, 12, 2, 7, 7, 9, 2), nrow=4, ncol=3, byrow=F)
...
```{r}
D <- data.frame(vector=v, color=c("red", "blue", "green", "grey", "orange", "purple", "brown"))
D
DF <- data.frame(M, col=c("A", "B", "C", "D"))
DF
```
...
for(i in 1:4){ 
    print(mean(M4[i, ]))
} #for every of the 4 rows of my matrix, I ask to print the mean
```
...
```{r}
plot(M4[,1], M4[,3], xlab = "labelx", ylab= "labely", col = "red")
```
...
```{r}
plot(M["gene1", ], M["gene3", ])
abline(h=11) # playing a bit with abline
legend(4, 10, legend="Mdata") # playing a bit with legend

plot(M["gene1",], M["gene3",], col="blue", pch=23, cex=3) # playing a bit with the arguments you can find in ?plot and ?var
...
write.table(M, file="expression_data.txt")
dir()
```
