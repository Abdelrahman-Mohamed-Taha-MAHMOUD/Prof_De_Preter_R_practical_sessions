...
```{r}
expression_data <- read_tsv(file = "qPCR_expression_data.tsv")
sample_data <- read_tsv(file = "qPCR_sample_data.tsv", col_types = "cfcffdd", col_names = TRUE)
```
...
t.test(length~group) # parametric test
wilcox.test(length~group) # non-parametric test
```
...
#calculate means
expression_data_pergene %>%
    summarise(mean(gene39), mean(gene46))

#plot density gene 39
expression_data_pergene %>%
  ggplot(aes(gene39))+
  geom_density()+
  theme_classic()

#plot density gene 46
expression_data_pergene %>%
  ggplot(aes(gene46))+
  geom_density()+
  theme_classic()
...
t.test(expression_data_pergene$gene39, expression_data_pergene$gene46, paired=T) # a paired test is more interesting to us in this case, as the same samples/patients were tested for gene39 and for gene46 expression.

#perform non-parametric test
wilcox.test(expression_data_pergene$gene39, expression_data_pergene$gene46, paired=T)
...
For this exercise, you have to put expression data (where you can find gene14, gene39 and gene45 expression) and the sample data (where you can find MYCN amplification status) together in one tibble. You can obtain this using the function left_join. This makes sure that the patient order is the same for all variables you're gonna use in your statistical test. Check this table using the head-function.

```{r}
all_expression_sample_data <- left_join(expression_data_pergene, sample_data, by=c("name"="patientID"))
head(all_expression_sample_data)
```
...
t.test(all_expression_sample_data$gene14~all_expression_sample_data$MNA)
t.test(all_expression_sample_data$gene39~all_expression_sample_data$MNA)
t.test(all_expression_sample_data$gene45~all_expression_sample_data$MNA)

# perform non-parametric tests
wilcox.test(all_expression_sample_data$gene14~all_expression_sample_data$MNA)
wilcox.test(all_expression_sample_data$gene39~all_expression_sample_data$MNA)
wilcox.test(all_expression_sample_data$gene45~all_expression_sample_data$MNA)

# draw boxplots
boxplot(all_expression_sample_data$gene14~all_expression_sample_data$MNA, main="gene14 expression in MYCN amp (1) vs non MYCN amp (1) samples")

all_expression_sample_data %>%
    ggplot(aes(x=as.factor(MNA), y=gene14))+
    geom_boxplot()+
    theme_classic()
```
...
#plot scatter
expression_data_pergene %>%
    ggplot(aes(x=gene39,y=gene46))+
        geom_point()+
        theme_classic()

#calculated Pearson and Spearman correlation
cor.test(expression_data_pergene$gene39, expression_data_pergene$gene46) #default: the Pearson correlation will be calculated
cor.test(expression_data_pergene$gene39, expression_data_pergene$gene46, method="spearman")
```
...
# gene 39
# fit the model
lm_stage_mycn <- lm(gene39 ~ Stage, data=all_expression_sample_data)
# gives you anova test results for the model
anova(lm_stage_mycn)
# gives you results for each value of 'stage'
summary(lm_stage_mycn)

ggplot(all_expression_sample_data, aes(x = Stage, y = gene39, group= Stage, fill=Stage)) +
  geom_boxplot() +
  geom_point(position = "jitter", color="grey")+
  theme_classic()

# kruskal wallis test
kruskal.test(gene39 ~ Stage, data=all_expression_sample_data)

#gene 14
# plot the model
lm_stage_mycn <- lm(gene14 ~ Stage, data=all_expression_sample_data)
# gives you anova test results for the model
anova(lm_stage_mycn)
# gives you results for each value of 'stage'
summary(lm_stage_mycn)

ggplot(all_expression_sample_data, aes(x = Stage, y = gene14, group= Stage, fill=Stage)) +
  geom_boxplot() +
  geom_point(position = "jitter", color="grey")+
  theme_classic()

# kruskal wallis test
kruskal.test(gene14 ~ Stage, data=all_expression_sample_data)
```
