...
```{r}
NB_data_exp <- read.table("C:/Users/abdoj/OneDrive - UGent/Desktop/0. UGent-ABDO/1. J000530A -Data Intelligence in Sustainable Drug Discovery/3. Prof; De Preter - R practical sessions/2. Data visualisations/qPCR_expression_data.tsv", header = TRUE, sep = "\t")
NB_data_info <- read.table("C:/Users/abdoj/OneDrive - UGent/Desktop/0. UGent-ABDO/1. J000530A -Data Intelligence in Sustainable Drug Discovery/3. Prof; De Preter - R practical sessions/2. Data visualisations/qPCR_sample_data.tsv", header = TRUE, sep = "\t")
  
  # Read in the data from the TSV file
#qPCR_data <- read.table("qPCR_data.tsv", header = TRUE, sep = "\t")
...
# Exclude the GeneName column
numeric_data <- NB_data_exp[, -1]

# Set the gene names as row names
rownames(numeric_data) <- NB_data_exp$GeneName

# Convert the data to a numeric matrix
numeric_data_matrix <- as.matrix(numeric_data)

# Load the required library if not already loaded
library(gtools)

# Create the heatmap
heatmap(numeric_data_matrix,
        scale = "row",  # Scale the data by rows for better comparison
...
custom_colors <- colorRampPalette(c("darkred", "white", "darkgreen"))(50)
...
heatmap(numeric_data_matrix,
        ...
        xlab = "Samples",
        ylab = "Genes",
        col = custom_colors,  # Custom color scheme
        breaks = seq(-5, 5, length.out = 51))  # Define breaks from -5 to 5
...
heatmap(numeric_data_matrix,
        ...
        Colv = NA,       # Do not cluster columns
        main = "Gene Expression Heatmap",
        xlab = "",  # Custom column title (empty string to remove)
        ylab = "",  # Custom row title (empty string to remove)
        col = custom_colors,  # Custom color scheme
        breaks = seq(-5, 5, length.out = 51))  # Define breaks from -5 to 5
...
# Calculate Manhattan correlation distance
row_dist <- dist(numeric_data_matrix, method = "manhattan")
col_dist <- dist(t(numeric_data_matrix), method = "manhattan")

# Cluster the data using hclust
row_cluster <- hclust(row_dist)
col_cluster <- hclust(col_dist)

# Split the patients into three groups
num_patients <- ncol(numeric_data_matrix)
num_groups <- 3
group_assignment <- factor(rep(1:num_groups, each = ceiling(num_patients / num_groups)), labels = 1:num_groups)

# Create the heatmap with custom colors, clustering, and group assignment
heatmap(numeric_data_matrix,
        scale = "row",  # Scale the data by rows for better comparison
        Colv = as.dendrogram(col_cluster),  # Column clustering
        Rowv = as.dendrogram(row_cluster),  # Row clustering
        main = "Gene Expression Heatmap",
        xlab = "",  # Custom column title (empty string to remove)
        ylab = "",  # Custom row title (empty string to remove)
        col = custom_colors,  # Custom color scheme
        breaks = seq(-5, 5, length.out = 51),  # Define breaks from -5 to 5
        ColSideColors = as.character(group_assignment))  # Color patients by group
...
# Create the heatmap with custom colors and without dendrograms
heatmap(numeric_data_matrix,
        scale = "row",  # Scale the data by rows for better comparison
        Colv = NULL,  # Remove column dendrogram
        Rowv = NULL,  # Remove row dendrogram
        main = "Gene Expression Heatmap",
        xlab = "",  # Custom column title (empty string to remove)
        ylab = "",  # Custom row title (empty string to remove)
        col = custom_colors,  # Custom color scheme
...
annotation_data <- read.table("C:/Users/abdoj/OneDrive - UGent/Desktop/0. UGent-ABDO/1. J000530A -Data Intelligence in Sustainable Drug Discovery/3. Prof; De Preter - R practical sessions/2. Data visualisations/qPCR_expression_data.tsv", header = TRUE, sep = "\t")

# Ensure that the MNA status and stage columns are factors
annotation_data$MNA_Status <- as.factor(annotation_data$MNA_Status)
annotation_data$Stage <- as.factor(annotation_data$Stage)

# Exclude the GeneName column from the expression data
numeric_data <- NB_data_exp[, -1]
...
# Create the heatmap with custom colors, without dendrograms, and with patient annotations
heatmap(numeric_data_matrix,
        scale = "row",  # Scale the data by rows for better comparison
        Colv = NULL,  # Remove column dendrogram
        Rowv = NULL,  # Remove row dendrogram
        ...
        col = custom_colors,  # Custom color scheme
        breaks = seq(-5, 5, length.out = 51),  # Define breaks from -5 to 5
        ColSideColors = as.matrix(annotation_data[, c("MNA_Status", "Stage")]))
